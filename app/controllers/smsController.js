...
const sendSMSOTP = async (req, res) => {
  try {
    ...
    await OTP.destroy({ where: { phoneNumber: cleanPhoneNumber, isVerified: false, channel: 'sms' } });
    ...
    res.status(500).json({ success: false, message: 'Failed to send SMS OTP', error: error.message });
  }
};
  ...
  try {
    ...
    if (otpRecord.otpCode === otpCode) {
      ...
      if (!user) {
        ...
        await user.update({ isVerified: true, preferredChannel: 'sms' });
      }
...
const resendSMSOTP = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'SMS OTP resent successfully', channel: 'sms' });
  } catch (error) {
